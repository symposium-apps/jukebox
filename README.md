...
Use **Import** beside Upload and Folder to paste a public YouTube or YouTube Music track, album, or playlist link. Jukebox inspects the source before downloading, lets you choose individual available tracks, audio-only MP3 or video output, quality, artwork, and the destination album or playlist, then keeps progress visible in a persistent Downloads drawer. A video import atomically adds the original MP4 and a separate MP3 audio copy from the same download. Jukebox then prepares a private six-second HLS VOD stream in background state while byte-range MP4 playback remains immediately available. When an existing playlist is selected for a video import, that playlist receives one video entry; the paired MP3 remains independently available in the library without creating a duplicate playlist row. Playlist imports retain per-track metadata and can create or update a Jukebox playlist.

Accepted media-link forms include standard, mobile, and YouTube Music watch URLs; `youtu.be` shares; Shorts; Live links; YouTube and privacy-enhanced embeds; legacy `/v/` embeds; playlists, podcasts, clips, and old attribution links. Tracking parameters are removed and supported links are canonicalized before inspection and deduplication. Channel pages, searches, redirects, non-HTTPS URLs, and lookalike hosts remain rejected.
