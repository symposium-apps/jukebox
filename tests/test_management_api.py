...
class ManagementApiTest(unittest.TestCase):
    ...
    def test_complete_authenticated_management_flow(self) -> None:
        ...
        self.assertIn(b'<meta name="jukebox-version" content="1.4.8">', page)
        self.assertNotIn(b"__JUKEBOX_VERSION__", page)
        self.assertIn(b'<link rel="stylesheet" href="/static/link-import.css?v=1.4.8">', page)
        self.assertIn(b'<link rel="stylesheet" href="/static/media-player.css?v=1.4.8">', page)
        self.assertIn(b'<script src="/static/link-import.js?v=1.4.8"></script>', page)
        self.assertIn(b'<script src="/static/logical-media.js?v=1.4.8"></script>', page)
        self.assertIn(b'<script src="/static/media-player.js?v=1.4.8"></script>', page)
        ...
        self.assertIn(b'video.addEventListener("canplay"', player_js)
        self.assertIn(b"!videoSourceIsHls && typeof video.fastSeek", player_js)
        self.assertIn(b"hlsStartupSyncPending && video.currentTime > 0.2", player_js)
        self.assertIn(b"hlsStartupSyncPending = false;", player_js)
        ...
        self.assertEqual(hls_clock_branch.count(b"seekVideo("), 1)
