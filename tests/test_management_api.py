...
class ManagementApiTest(unittest.TestCase):
    ...
    def test_complete_authenticated_management_flow(self) -> None:
        ...
        self.assertIn(b'<meta name="jukebox-version" content="1.4.13">', page)
        self.assertNotIn(b"__JUKEBOX_VERSION__", page)
        self.assertIn(b'<link rel="stylesheet" href="/static/link-import.css?v=1.4.13">', page)
        self.assertIn(b'<link rel="stylesheet" href="/static/media-player.css?v=1.4.13">', page)
        self.assertIn(b'<script src="/static/link-import.js?v=1.4.13"></script>', page)
        self.assertIn(b'<script src="/static/logical-media.js?v=1.4.13"></script>', page)
        self.assertIn(b'<script src="/static/media-player.js?v=1.4.13"></script>', page)
        ...
        self.assertIn(b"grid-template-columns: repeat(5,44px)", page)
        self.assertIn(b"data-format-track", page)
        self.assertIn(b"selectedVariantIds", page)
        css_status, css_headers, css = self.request("GET", "/static/link-import.css")
        self.assertEqual(css_status, 200)
        self.assertEqual(css_headers["Content-Type"], "text/css; charset=utf-8")
        self.assertIn(b".jbi-modal--playlist", css)
        js_status, js_headers, js = self.request("GET", "/static/link-import.js")
        self.assertEqual(js_status, 200)
        self.assertEqual(js_headers["Content-Type"], "text/javascript; charset=utf-8")
        self.assertIn(b'api("/api/import/inspect"', js)
        player_status, player_headers, player_js = self.request("GET", "/static/media-player.js")
        self.assertEqual(player_status, 200)
        self.assertEqual(player_headers["Content-Type"], "text/javascript; charset=utf-8")
        self.assertIn(b"audioOnlyBtn", player_js)
        self.assertIn(b"scrubSlider", player_js)
        self.assertIn(b'qs("nextBtn").insertAdjacentElement("afterend", showVideo)', player_js)
        self.assertIn(b"video-return-control", player_js)
        self.assertIn(b'id="videoRewindBtn"', player_js)
        self.assertIn(b'id="videoPlayPauseBtn"', player_js)
        self.assertIn(b'id="videoForwardBtn"', player_js)
        self.assertIn(b'id="videoScrubSlider"', player_js)
        self.assertIn(b'aria-label="Rewind 10 seconds"', player_js)
        self.assertIn(b'aria-label="Forward 10 seconds"', player_js)
        self.assertIn(b"Repeated currentTime assignments on native HLS", player_js)
        self.assertIn(b"!videoSourceIsHls && typeof video.fastSeek", player_js)
        self.assertIn(b"async function alignHlsPair", player_js)
        self.assertIn(b"waitUntilPlayable(video", player_js)
        self.assertIn(b"waitUntilPlayable(audio", player_js)
        self.assertIn(b"Requiring HAVE_FUTURE_DATA here deadlocks", player_js)
        self.assertIn(b'audio.addEventListener("waiting", recoverBufferedHlsPair)', player_js)
        self.assertIn(b'video.addEventListener("waiting", recoverBufferedHlsPair)', player_js)
        self.assertIn(b"Do not hard-seek the HLS element during a transient stall", player_js)
        self.assertIn(b"recoverBufferedHlsPair();", player_js)
        self.assertIn(b"Align the paused audio clock", player_js)
        self.assertIn(b"fastSeek(). Assigning currentTime triggers", player_js)
        self.assertNotIn(b"typeof audio.fastSeek", player_js)
        self.assertNotIn(b"Math.abs(video.currentTime - audio.currentTime) > 0.35", player_js)
        hls_clock_branch = player_js.split(b"} else if (videoSourceIsHls) {", 1)[1].split(b"} else if (!Number.isFinite(drift)", 1)[0]
        self.assertIn(b"video.playbackRate", hls_clock_branch)
        self.assertNotIn(b"seekVideo(", hls_clock_branch)
        player_css_status, player_css_headers, player_css = self.request("GET", "/static/media-player.css")
        self.assertEqual(player_css_status, 200)
        self.assertEqual(player_css_headers["Content-Type"], "text/css; charset=utf-8")
        self.assertIn(b".video-transport", player_css)
        self.assertIn(b"min-height:44px", player_css)
        logical_status, logical_headers, logical_js = self.request("GET", "/static/logical-media.js")
        self.assertEqual(logical_status, 200)
        self.assertEqual(logical_headers["Content-Type"], "text/javascript; charset=utf-8")
        self.assertIn(b"groupTracks", logical_js)
        self.assertIn(b"format_variants", logical_js)
        self.assertEqual(self.json_request("GET", "/api/import/jobs")[0], 200)
        self.assertEqual(self.json_request("GET", "/api/v1/imports/jobs")[0], 401)
        invalid_import = self.json_request("POST", "/api/import/inspect", {"url": "https://example.com/watch?v=abc"})
        self.assertEqual(invalid_import[0], 400)
        self.assertIn("not a supported", invalid_import[2]["error"])
        ...
        self.assertEqual(manifest["id"], "/jukebox-pwa")
        self.assertEqual(manifest["start_url"], "/app?source=pwa")
        self.assertEqual(manifest["scope"], "/")
        self.assertEqual(manifest["display"], "standalone")
        self.assertEqual(manifest["name"], "Jukebox")
        self.assertEqual(self.request("GET", manifest["start_url"])[0], 200)
...
class StartupCompatibilityTest(unittest.TestCase):
    ...
    def test_browser_player_publishes_media_session_metadata_and_controls(self) -> None:
        ...
        self.assertNotIn('mediaSessionMetadataKey !== "ios-document"', page)
