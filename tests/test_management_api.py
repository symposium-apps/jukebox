...
class ManagementApiTest(unittest.TestCase):
    ...
    def test_complete_authenticated_management_flow(self) -> None:
        ...
        self.assertIn(b"The complete visual player is deliberately owned by this isolated module", player_js)
        self.assertIn(b"document.body.appendChild(showVideo)", player_js)
        self.assertNotIn(b'qs("nextBtn").insertAdjacentElement("afterend", showVideo)', player_js)
        ...
        self.assertIn(b"Jukebox cinematic video player", player_css)
        self.assertIn(b".video-return-control", player_css)
        self.assertIn(b"width: 46px", player_css)
        self.assertIn(b"width: 100% !important", player_css)
